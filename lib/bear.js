...
exports.deploy = function( conf ) {
    ...
    var assetsDir = path.resolve( config.path.www, config.path.assets );
    console.log( assetsDir );
    console.log( path.resolve( config.path.assets ) );
    // Copy complete assets folder TODO: should later be done using grunt
    /*if ( !fs.existsSync( assetsDir ) ) {
        fs.mkdir( assetsDir );
    }*/
    wrench.copyDirSyncRecursive( path.resolve( config.path.assets ), assetsDir, {
        forceDelete: true
    } );
